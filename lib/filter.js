...
function logic_js(str, data) {
    ...
    // const result = UglifyJS.minify(str, uglify_options);
    // if (!result.error) {
    ...
    // return js_result;
    terser.minify(str, uglify_options).then((result) => {
        const saved = ((str.length - result.code.length) / str.length * 100).toFixed(2);
        if (options.logger) {
            const log = hexo.log || console.log;
            if (path === undefined) {
        ...
        if (options.stamp) {
            const prefix = '// build time:' + Date().toLocaleString() + "\n";
            const end = '\n//rebuild by hexo-renderer-multi-next-markdown-it (terser ver.)';
            js_result = prefix + js_result + end;
        }
        return js_result;
    }).catch((reason => {
